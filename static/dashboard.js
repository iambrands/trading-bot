...
async function getBacktestAIAnalysis(results) {
    ...
    try {
        ...
        analysisContent.innerHTML = `<div class="error">AI analysis error: ${escapeHtml(errorMessage)}<br><small>Please check that CLAUDE_API_KEY is configured in Railway environment variables</small></div>`;
...
async function getAIHelp(topic, context = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            showToast(`AI help: ${error.error || 'Please configure CLAUDE_API_KEY in Railway environment variables'}`, 'warning');
            return;
        }
        ...
        showToast('AI help unavailable. Configure CLAUDE_API_KEY in Railway environment variables', 'warning');
    }
}
